...
io.on('connection', socket => {
  ...
  io.to(roomName).emit('connectUser', getUsers(roomName)); // 메시지를 받으면
...
app.get("/getRooms", (req, res) => {
  MongoClient.connect(mongo_HOST, function (err, db) {
    if (err) throw err;
    database = db.db(DB_NAME);
    database.collection('chatingRooms').find({}).sort({
      _id: -1
...
app.post("/addRooms", (req, res) => {
  ...
  MongoClient.connect(mongo_HOST, function (err, db) {
    if (err) throw err;
    database = db.db(DB_NAME);
    database.collection('chatingRooms').insertOne({
      roomName: roomName
    }, function (err, res) {
      if (err) throw err;
      db.close();
...
app.post('/getLogs', (req, res) => {
  ...
  MongoClient.connect(mongo_HOST, function (err, db) {
    if (err) throw err;
    database = db.db(DB_NAME);
    database.collection('chatingLog').find({
      roomName: roomName
    }).sort({
      _id: -1
    }).toArray(function (err, result) {
